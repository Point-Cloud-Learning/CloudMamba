...
def build_from_cfg(cfgs, registry, default_args=None):
    """Build a module from config dict.
    Args:
        cfgs (edict): Config dict. It should at least contain the key "NAME".
        registry (:obj:`Registry`): The registry to search the type from.
    Returns:
        object: The constructed object.
    """
    if not isinstance(cfgs, dict):
        raise TypeError(f'cfg must be a dict, but got {type(cfgs)}')
    if 'NAME' not in cfgs:
    ...
    if default_args is not None:
        cfgs = Config.merge_new_config(cfgs, default_args)

    obj_type = cfgs.get('NAME')
